package aufgabe3.mnistReadClasses;
...
public class MnistDataReader  {
    ...
    public static DataPoint[] readDataIntoDataPoints(String dataFilePath, String labelFilePath) throws IOException {
        DataInputStream dataInputStream = new DataInputStream(new BufferedInputStream(new FileInputStream(dataFilePath)));
        int magicNumber = dataInputStream.readInt();
        int numberOfItems = dataInputStream.readInt();
        int nRows = dataInputStream.readInt();
        ...
        for (int i = 0;i<numberOfItems;i++){
            double[] inputData = new double[nRows*nCols];
            for (int n = 0;n<nRows*nCols;n++){
                inputData[n] = dataInputStream.readUnsignedByte();
                inputData[n] = inputData[n] / 255.0;
            }
            data[i] = new DataPoint(inputData,labelInputStream.readUnsignedByte(),10);
        }
        dataInputStream.close();
        labelInputStream.close();
        return data;
